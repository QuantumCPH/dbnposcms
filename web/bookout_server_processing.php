 ...
 */
$aColumns = array('bookout_notes.note_id',   'bookout_notes.branch_number','bookout_notes.status_id', 'bookout_notes.synced_at','bookout_notes.received_at','bookout_notes.updated_by', 'bookout_notes.id');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "bookout_notes.id";

/* DB table to use */
$sTable = "bookout_notes";


$sJoin = ' LEFT JOIN items   ON items.item_id = bookout_notes.item_id ';
...
if ($_GET['sSearch'] != "") {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
     ...
     $sWhere .=  "  bookout_notes.item_id LIKE '%" . mysql_real_escape_string($_GET['sSearch']) . "%' OR ";
...
$sWhere .=" Group by bookout_notes.note_id";
/*
 * SQL queries
 * Get data to display
 */
...
$sQuery1 = "
	  ...
	  Group by bookout_notes.note_id";
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($col[1] == "note_id") {
            ...
            $vartal = $aRow[$col[1]];
            $row[] = "<a href=".$siteUrl."backend.php/bookoutNotes/view?id=" . $aRow['note_id'] . ">" . $vartal . " </a>";
        } else if ($col[1] == 'id') {
//            $row[] = "here";
            $sq = "Select sum(is_received) from " . $sTable . " where note_id = '" . $aRow['note_id'] . "'";
            $rs = mysql_query($sq, $gaSql['link']);
            $actions ="";
            while ($arr = mysql_fetch_array($rs)) {
                if ($arr[0] == 0) {                    
                    $actions .= "<a href=".$siteUrl."backend.php/bookoutNotes/edit?id=" . $aRow['note_id'] . " ><img src='".$siteUrl."sf/sf_admin/images/edit_icon.png' /></a>";
                   // $actions .= "<a href=".$siteUrl."backend.php/bookoutNotes/deleteNotes/id/" . $aRow['note_id'] . "  onclick=\"return confirm('Are you sure to delete delivery notes?');\"> <img src='".$siteUrl."sf/sf_admin/images/delete.png' /> </a>";
