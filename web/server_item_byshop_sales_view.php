...
$row[] =$abc;
            ...
            }elseif ($aColumns[$i] == "transactions.transaction_type_id") {
                $query="select title from transaction_types where id=".$aRow[$col[1]];
                $rRs = mysql_query($query, $gaSql['link']) or die(mysql_error());
                $atype = mysql_fetch_array($rRs);
                if($atype['title']){
                 $row[] =$atype['title'];
                }else{
                   $row[]=""; 
