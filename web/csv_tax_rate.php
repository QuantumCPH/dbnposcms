...
$sQuery1 = ' 
SELECT   "Tax" AS taxval, "' . $vatValue . ' %" AS vatval,    round(sum((sold_price)*' . $vatValue . '/100),2) as vatprice,round(sum(sold_price),2) as soldprice  FROM transactions     WHERE     transactions.status_id=3 AND (transactions.transaction_type_id=3 OR transactions.transaction_type_id=2)   AND transactions.created_at>="' . $startDate . '"   AND transactions.created_at<="' . $endDate . '"';
