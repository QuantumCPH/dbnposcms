 ...
 */
$aColumns = array('image_history.image_name', 'image_history.created_at','user.name');

/* Indexed column (used for fast and accurate table cardinality) */
$sIndexColumn = "id";

/* DB table to use */
$sTable = "image_history";
$sJoin  = ' LEFT JOIN user   ON user.id = image_history.updated_by ';
...
if($sOrder==""){
    $sOrder = "ORDER BY image_history.id Desc";
}
/*
 * Filtering
...
for ($i = 0; $i < count($aColumns); $i++) {
 ...
 if ($sWhere == "") {
            $sWhere = "WHERE  image_history.item_id='".$_GET['item_id']."'" ;
        } else {
            $sWhere .= " AND  image_history.item_id='".$_GET['item_id']."'" ;
        }
...
while ($aRow = mysql_fetch_array($rResult)) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
          $col = explode('.',$aColumns[$i]);
           ...
           if ($aColumns[$i] == "image_history.image_name") {
              $aRow[$col[1]];
