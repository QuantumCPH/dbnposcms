 ...
 */
$aColumns = array("shops.branch_number", "inventory.total", "inventory.sold", "inventory.book_out", "inventory.returned", "inventory.available","inventory.delivery_count","inventory.item_id");
...
for ($i = 0; $i < count($aColumns); $i++) {
 ...
 if ($sWhere == "") {
            $sWhere = "WHERE  inventory.item_id='".$_GET['item_id']."'" ;
        } else {
            $sWhere .= " AND  inventory.item_id='".$_GET['item_id']."'" ;
        }
