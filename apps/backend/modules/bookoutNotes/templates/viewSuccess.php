...
<div class="itemslist">
    ...
    <h1 class="items-head list-page">
        <div class="titleicon"><img src="<?php echo sfConfig::get('app_web_url') . 'images/delivery_notes_title.png' ?>" />&nbsp;Bookout Note Details</div>
...
<div  class="backimgDiv">
       ...
       <?php if($edit_del):?>  
          <input type="button" onclick="document.location.href='<?php echo sfConfig::get("app_admin_url");?>bookoutNotes/edit/?id=<?php echo $id;?>';" value="Edit" class="btn btn-primary"/>
               <?php endif;?>
          
<?php if(gf::getBPreviousDNId($dn->getGroupId())){ ?>
           <a href="<?php echo url_for("bookoutNotes/view?id=".gf::getBPreviousDNId($dn->getGroupId())) ?>" class="btn btn-primary viewPrev">Previous </a>
        <?php } ?>         
 <?php if(gf::getBNextDNId($dn->getGroupId())){?>
           <a href="<?php echo url_for("bookoutNotes/view?id=".gf::getBNextDNId($dn->getGroupId()))?>" class="btn btn-primary viewNext">Next </a>
        <?php } ?>
...
<div class="itemslist listview" style="">  <br />
    ...
    <table cellspacing="0" class="deliverinfotbl">
        ...
        <tr>
            <td>Bookout  Date:</td><td><label><?php echo date("Y-m-d",strtotime($delivery_date)); ?></label></td>
        </tr>
        <tr>
